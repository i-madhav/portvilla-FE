...
function IdentityEdit({
  ...
  return (
    <form onSubmit={submit}>
      ...
      <input value={name} onChange={(e) => setName(e.target.value)} style={inputStyle(!name.trim())} />

      <label style={miniLabel}>Tagline</label>
      <input value={tagline} onChange={(e) => setTagline(e.target.value)} style={inputStyle(false)} maxLength={120} />

      <label style={miniLabel}>Bio</label>
      <textarea value={bio} onChange={(e) => setBio(e.target.value)} style={textareaStyle(false)} rows={2} maxLength={500} />

      <label style={miniLabel}>About</label>
      <textarea value={about} onChange={(e) => setAbout(e.target.value)} style={textareaStyle(false)} rows={4} maxLength={2000} />

      <div style={{ display: 'flex', gap: '0.75rem' }}>
        <div style={{ flex: 1 }}>
          <label style={miniLabel}>Location</label>
          <input value={location} onChange={(e) => setLocation(e.target.value)} style={inputStyle(false)} />
        </div>
        <div style={{ flex: 1 }}>
          <label style={miniLabel}>Industry</label>
          <input value={industry} onChange={(e) => setIndustry(e.target.value)} style={inputStyle(false)} />
        </div>
      </div>

      <label style={miniLabel}>Availability</label>
      <input value={availability} onChange={(e) => setAvailability(e.target.value)} style={inputStyle(false)} />
